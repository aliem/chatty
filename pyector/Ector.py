...
class Ector:
    ...
    def __init__(self,botname="Ector",username="User"):
        ...
        if os.path.exists("cn.yml"):
            f    = open("cn.yml","r")
            self.cn    = yaml.load(f.read())
            f.close()
        else:
    ...
    def dump(self):
        ...
        Save the ConceptNetwork in cn.yml, and the state in usernameState.yml"""
        ...
        if self.username:
            ...
            f.close()
        return {"cn":fcn.closed, "un":f.closed}
    ...
    def __getStateId(self):
        ...
        return self.username + "_state.yml"
